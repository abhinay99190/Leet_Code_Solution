...
in altitude between points i and i + 1 for all (0 <= i < n). Return the highest altitude of a points
point.
...
Example 2:-- Input:-- gain = [-4,-3,-2,-1,4,3,2]
Output: 0
...
public class Solution {
    ...
    public static int largestAltitude(int[] gain) {
        ...
        for (int i = 0; i < gain.length; i++) {
            attitude = attitude + gain[i];
           min = Math.max(attitude, min);
        }
        return min;
    }
}
